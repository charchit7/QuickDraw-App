...
class MyDataset(Dataset):
    """
    Custom dataclass for QuickDraw dataset!
    """
    def __init__(self, root_path='data', total_images_per_classes=None,ratio=0.8,mode='train'):
        ...
        if mode == 'train':
            self.offset = 0
            self.num_images_per_class = int(total_images_per_classes * ratio)

        else:
            self.offset = int(total_images_per_classes * ratio)
            self.num_images_per_class = int(total_images_per_classes * (1-ratio))
